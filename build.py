...
TARGET = 'zh-CN'
# 目标语言的字符类，判定一行是否已是目标语言、译文是否真的译过去了。
# 换 TARGET 要跟着换这两条
TARGET_CHARS = '㐀-鿿'  # 汉字，含扩展 A 区
TARGET_EXCLUDE = '぀-ヿㇰ-ㇿ가-힣ｦ-ﾝ'  # 假名与谚文：日韩也写汉字，凭它们排除
...
PAIRING = [
    ...
    # 哨兵行的译文没有意义
    ('缩哨兵', r'(?m)^(⸻)\n[^\n]+$', '$1', True),
    # 译文里没有目标语言的字：这一行没真的译过去，只留原文。接口在检测不出源
    # 语言时会退化——音译回罗马字、原样回声、或者按猜错的语言漏译（末尾的词直
    # 接没了）。判据只看译文形态，不看原文长相，短回复（Ja、Ok）的译文照样带
    # 目标语言的字，不受牵连
    ('留原文', rf'(?m)^([^\n]+)\n(?![^\n]*[{TARGET_CHARS}])[^\n]+$', '$1', True),
